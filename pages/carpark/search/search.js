...
Page({
  ...
  findCar() {
    if (this.data.current === 'num') {
      ...
      // app.wechat.navigatorTo('../findlist/findlist?carnum=' + carnum);
      app.wechat.navigatorTo('../result/result?carnum=' + carnum);
    } else if (this.data.current === 'num2') {
      let carNum2 = this.data.carNum2;
      var carnum2 = carNum2.join('');;
      if (carnum2.indexOf('-') !== -1) {
        $Toast({
          content: '请输入后四位车牌号',
          type: 'error'
        });
      ...
      app.wechat.navigatorTo('../list/list?carnum=' + carnum2);
    } else {
      let _start = this.data.startdate;
      // let _start = this.data.startdate + ' 00:00:00';
      // let _end = this.data.enddate + ' ' + this.data.endtime + ':00';
      app.wechat.navigatorTo('../list/list?start=' + _start);
    }
  },
  onLoad: function(query) {
    ...
    //   wx.setStorageSync(app.hyzconst.SCAN_POINT, urlarr[urlarr.length - 1]);
    // }

    let myDate = new Date();
    let nowdate = myDate.toLocaleDateString().split('/').join('-');
    this.setData({
      startdate: nowdate,
      enddate: nowdate
    });
  ...
  getMyCar() {
    ...
    app.wechat.fetchAPI('POST', app.api.COMMON_API.parkinglot.USERCAR_LIST, params).then(res => {
      if (res.data.hyz_code === 20000) {
        if (res.data.hyz_result) {
          this.setData({
            mycars: res.data.hyz_result.list.map(c => c.licensePlateNumber)
          })
        }
      }
    });
  ...
  goPay() {
    // app.wechat.navigatorTo('../payfee/payfee?carnum=' + this.data.recordInfo.licensePlateNumber);
    app.wechat.navigatorTo('../result/result?carnum=' + this.data.recordInfo.licensePlateNumber);
  },
  /**
   * 生命周期函数--监听页面显示
   */
  ...
  bindMineChange:function(e) {
    let _selectCompany = this.data.mycars[e.detail.value];
    let carnumstr = _selectCompany;
    let _carNum = {
